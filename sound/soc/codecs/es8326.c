...
static const struct regmap_range es8326_volatile_ranges[] = {
	regmap_reg_range(ES8326_HP_OFFSET_CAL, ES8326_CSM_MUTE_STA),
};
...
static const struct _coeff_div coeff_div[] = {
	...
	{256, 44100, 11289600, 0x01, 0x00, 0x30, 0x2D, 0x26, 0x26, 0x1F, 0x1F},
	{256, 48000, 12288000, 0x01, 0x00, 0x30, 0x2D, 0x26, 0x26, 0x1F, 0x1F},
				...
				struct snd_soc_dai *dai)
{
	...
	coeff = get_coeff(es8326->sysclk, params_rate(params));  //params_rate(params)
	/* bit size */
...
static void es8326_jack_button_handler(struct work_struct *work)
{
	...
	iface = snd_soc_component_read32(comp, ES8326_HPDET_STA);
	switch (iface) {
	case 0x93:
...
static void es8326_jack_detect_handler(struct work_struct *work)
{
	...
	if (es8326->jack_remove_retry == 1) {
		...
		printk("remove event check, set HPJACK_POL normal, cnt = %d\n",
	...
	if ((iface & ES8326_HPINSERT_FLAG) == 0) {
		...
		printk("%s, No headset detected\n", __func__);
		es8326_disable_micbias(es8326->component);
		if (es8326->jack->status & SND_JACK_HEADPHONE) {
			printk("%s,report hp remove event\n", __func__ );
		...
		if(es8326->jack_remove_retry == 0) {
			...
			printk("remove event check, invert HPJACK_POL, cnt = %d\n",
					es8326->jack_remove_retry);
			regmap_update_bits(es8326->regmap, ES8326_HPDET_TYPE, 
					ES8326_HP_DET_JACK_POL, (es8326->jd_inverted ?
					es8326->jack_pol : ~es8326->jack_pol));
		...
		es8326->jack_remove_retry = 0;
		if(es8326->hp == 0) {
			printk("%s,First insert, start OMTP/CTIA type check\n", __func__ );
			/* 
...
static int es8326_calibrate(struct snd_soc_component *component)
{
	...
	if ((es8326->version == ES8326_VERSION_B) && (es8326->calibrated == false)) {
		pr_info("%s, version = %d, calibrating\n", __func__, es8326->version);
			    ...
			    const struct i2c_device_id *id)
{
	...
	regmap_read(es8326->regmap, ES8326_CHIP_ID2, &val);
	if (val != ES8326_DEVICE_ID) {
		dev_err(&i2c->dev,
			"Device with ID register %#x is not es8326/39\n", val);
		return -ENODEV;
	}
