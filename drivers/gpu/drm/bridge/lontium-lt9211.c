...
struct video_timing g_video[] = 
{
    ...
    { 58,  34,  88, 1920, 2100,   4,   5,  36, 1080, 1125, 148500},  // 1920x1080
...
static void LT9211_ChipID(struct i2c_client *client)
{
    ...
    do 
    {
        _LT9211_mipi_write( client, 0xff, 0x81);
    } while(0);
...
static void LT9211_TxDigital(struct i2c_client *client)
{
    if( (LT9211_OutPutModde == OUTPUT_LVDS_2_PORT) || (LT9211_OutPutModde == OUTPUT_LVDS_1_PORT) ) 
    {
        ...
        //LT9211_mipi_write( client,0x59,0x50); /* Bit5 disable for Sync mode */
	LT9211_mipi_write( client,0x59,0x70); /* Bit5 enable for DE mode */
...
//    msleep(3000);
	...
	if (gpio_is_valid(g_LT9211->pwm_pin))
        {
                lt9211_printk("enable pwm_pin\n");
                gpio_direction_output(g_LT9211->pwm_pin,1);
        }
...
static int LT9211_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    LT9211->pwm_pin = of_get_named_gpio_flags(client->dev.of_node, "pwm_gpio", 0,(enum of_gpio_flags *)&gpio_flags);
    ...
    if(LT9211->pwm_pin > 0)
    {
        if (!gpio_is_valid(LT9211->pwm_pin))
        {
            printk(" pwm_pin err %d %s \n",__LINE__,__func__);
            ret = -EINVAL;
        }
        else
        {
            ret = gpio_request(LT9211->pwm_pin, "pwm_pin");
            if (ret < 0) {
                printk("%s(): pwm_pin request failed %d\n",__func__, ret);
                return ret;
            }
        }
    }
