...
static struct lt9611uxc_mode *lt9611uxc_find_mode(const struct drm_display_mode *mode)
{
	...
	for (i = 0; i < ARRAY_SIZE(lt9611uxc_modes); i++) {
		if (lt9611uxc_modes[i].hdisplay == mode->hdisplay &&
		    ...
		    lt9611uxc_modes[i].clock == mode->clock &&
				...
				enum drm_bridge_attach_flags flags*/)
{
	...
	lt9611_printk("lt9611uxc_bridge_attach exit OK \n");
	return 0;

err_unregister_dsi0:
	mipi_dsi_device_unregister(lt9611uxc->dsi0);

lt9611_printk("lt9611uxc_bridge_attach exit Fail \n");
	return ret;
}
...
static int lt9611uxc_read_version(struct lt9611uxc *lt9611uxc)
{
	...
	ret = regmap_read(lt9611uxc->regmap, 0xb020, &rev);
	if (ret)
	{
		dev_err(lt9611uxc->dev, "failed to read sub revision: %d\n", ret);
	}
	else
	{
		dev_info(lt9611uxc->dev, "LT9611 sub version: 0x%02x\n", rev);
		lt9611uxc->fw_version = rev & 0xff;
	}
	...
	else
	{
		...
		lt9611uxc->fw_version |= (rev & 0xff) <<8;
	}
...
static char *lt9611uxc_firmware_read(struct lt9611uxc *lt9611uxc, size_t size)
{
	     ...
	     offset += LT9611UXC_FW_PAGE_SIZE)
	{
		...
		lt9611_printk("lt9611uxc_firmware_read remain = %ld \n", size-offset);
	}
...
static int lt9611uxc_firmware_update(struct lt9611uxc *lt9611uxc)
{
	     ...
	     offset += LT9611UXC_FW_PAGE_SIZE, remain -= LT9611UXC_FW_PAGE_SIZE)
	{
		...
		lt9611_printk("lt9611uxc_firmware_write_page remain = %ld \n", remain);
	}
...
out:
	...
	lt9611uxc_read_version(lt9611uxc);
	release_firmware(fw);
...
static ssize_t lt9611uxc_firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%x.%x.%x\n", (lt9611uxc->fw_version >> 12) & 0x0f,  (lt9611uxc->fw_version >> 8) & 0x0f, lt9611uxc->fw_version & 0xff);
}
