...
static int uart_mode_probe(struct platform_device *pdev)
{
	...
	sel0_gpio = of_get_named_gpio(np, "sel0_gpio", 0);
	sel1_gpio = of_get_named_gpio(np, "sel1_gpio", 0);
	term_gpio = of_get_named_gpio(np, "term_gpio", 0);
	slew_gpio = of_get_named_gpio(np, "slew_gpio", 0);
