...
/* Compatibility bits returned by RTC_VL_READ on older kernels. */
#define RV8803_VL_ACCURACY_LOW		BIT(0)
#define RV8803_VL_DATA_INVALID		BIT(1)
...
static bool rv8803_bcd_is_valid(u8 val)
{
...
static int rv8803_get_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (!rv8803_bcd_is_valid(alarmvals[0]) ||
	    !rv8803_bcd_is_valid(alarmvals[1]) ||
	    !rv8803_bcd_is_valid(alarmvals[2]))
		goto err_invalid;
...
static int rv8803_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	...
	case RTC_VL_READ:
		...
		if (flags & RV8803_FLAG_V1F) {
			dev_warn(&client->dev, "Voltage low, temperature compensation stopped.\n");
			vl |= RV8803_VL_ACCURACY_LOW;
		}

		if (flags & RV8803_FLAG_V2F)
			vl |= RV8803_VL_DATA_INVALID;
...
static const struct rtc_class_ops rv8803_rtc_ops_noalarm = {
	.read_time = rv8803_get_time,
	.set_time = rv8803_set_time,
	.ioctl = rv8803_ioctl,
};
			...
			const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		if (!id)
			id = i2c_match_id(rv8803_id, client);
	...
	if (client->irq > 0)
		...
		rv8803->rtc->ops = &rv8803_rtc_ops_noalarm;
	...
	err = rtc_nvmem_register(rv8803->rtc, &nvmem_cfg);
	if (err)
		return err;
