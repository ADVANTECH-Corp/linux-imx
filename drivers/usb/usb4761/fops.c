...
int daq_ioctl_dev_get_desc(daq_device_t *daq_dev, unsigned long arg)
{
   ...
   len = sprintf(desc, DEVICE_NAME", BID#%d", daq_dev->shared.BoardId);
...
int daq_ioctl_dev_set_board_id(daq_device_t *daq_dev, unsigned long arg)
{
   ...
   if (unlikely(daq_usb_dev_set_board_id(daq_dev, id) < 0)){
      return -EIO;
   }
...
int daq_file_close(struct inode *inode, struct file *filp)
{
   ...
   if (ctx->write_access) {
      for (i = 0; i < DI_INT_SRC_COUNT; ++i) {
         if (daq_dev->shared.IntCsr & (0x1 << i)) {
            daq_usb_enable_hw_event(daq_dev, i, 0, 0, 0);
         }
      }

      spin_lock_irqsave(&daq_dev->dev_lock, flags);
      if (daq_dev->shared.IntCsr){
         daq_dev->shared.IntCsr = 0;
         if (is_reader_running(&daq_dev->usb_reader)){
            spin_unlock_irqrestore(&daq_dev->dev_lock, flags);
