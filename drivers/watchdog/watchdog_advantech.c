...
static void adv_wdt_i2c_shutdown(struct i2c_client *client)
{
	...
	/*
	 * adv_wdt_stop() already cleared STARTED before this runs, so only
	 * system_state tells restart and poweroff/halt apart here; re-arm on
	 * restart only, keep WDT_EN/WDT_TRIG inactive on poweroff/halt.
	 */
