...
static void imx6_pcie_deassert_core_reset(struct imx6_pcie *imx6_pcie)
{
	...
	if (gpio_is_valid(imx6_pcie->power_on_gpio_5g))
		gpio_set_value_cansleep(imx6_pcie->power_on_gpio_5g, 1);
#endif
...
static int imx6_pcie_probe(struct platform_device *pdev)
{
	...
	imx6_pcie->power_on_gpio_5g = of_get_named_gpio(node, "power-on-gpio-5g", 0);
	if (gpio_is_valid(imx6_pcie->power_on_gpio_5g)) {
		ret = devm_gpio_request_one(&pdev->dev,
					    imx6_pcie->power_on_gpio_5g,
					    GPIOF_OUT_INIT_LOW,
					    "m.2 5g power enable");
		if (ret) {
			dev_err(&pdev->dev, "unable to get power-on m.2 5g\n");
			return ret;
		}
	} else if (imx6_pcie->power_on_gpio_5g == -EPROBE_DEFER) {
		return imx6_pcie->power_on_gpio_5g;
	}
	...
	imx6_pcie->minipcie_reset_gpio = of_get_named_gpio(node, "minipcie-reset-gpio", 0);
	if (gpio_is_valid(imx6_pcie->minipcie_reset_gpio)) {
		ret = devm_gpio_request_one(&pdev->dev,
					    imx6_pcie->minipcie_reset_gpio,
					    GPIOF_OUT_INIT_HIGH,
					    "minipcie-reset");
		if (ret) {
			dev_err(&pdev->dev, "unable to get usb-host-pwr-en gpio\n");
	...
	} else if (imx6_pcie->minipcie_reset_gpio == -EPROBE_DEFER) {
		return imx6_pcie->minipcie_reset_gpio;
	}
