...
void print_usage(char *prg)
{
	...
	fprintf(stderr, "         -s <speed>  (set CAN/CANFD speed 0..8)\n");
	fprintf(stderr, "         -d <dspeed> (set CANFD data speed 0..A)\n");
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "ocfls:d:S:t:b:?hF")) != -1) {
		switch (opt) {
		case 'o':
			send_open = 1;
			break;
	...
	if (name) {
		...
		if (s < 0)
            ...
            //if the device want to open, set the socket to up
            if (send_open) {
