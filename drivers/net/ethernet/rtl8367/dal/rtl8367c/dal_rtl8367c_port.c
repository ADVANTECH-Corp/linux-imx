...
rtk_api_ret_t dal_rtl8367c_port_macForceLinkExt_set(rtk_port_t port, rtk_mode_ext_t mode, rtk_port_mac_ability_t *pPortability)
{
    ...
    chipID = rtk_switch_chipType_get();
    ...
    if ((mode == MODE_EXT_HSGMII) || (mode == MODE_EXT_FIBER_2P5G))
    {
    ...
    else
    {
        ...
        {
            if((mode == MODE_EXT_HSGMII) || (mode == MODE_EXT_FIBER_2P5G))
                ability.speed = 5; //5:PORT_SPEED_2500M
            else
                ability.speed = pPortability->speed;
        }            
        else                
            ability.speed     = ((mode == MODE_EXT_HSGMII) || (mode == MODE_EXT_FIBER_2P5G)) ? PORT_SPEED_1000M : pPortability->speed;
...
rtk_api_ret_t dal_rtl8367c_port_macForceLinkExt_get(rtk_port_t port, rtk_mode_ext_t *pMode, rtk_port_mac_ability_t *pPortability)
{
    ...
    pPortability->speed     = ((*pMode == MODE_EXT_HSGMII) || (*pMode == MODE_EXT_FIBER_2P5G)) ? PORT_SPEED_2500M : ability.speed;
    pPortability->forcemode = ability.forcemode;
    pPortability->duplex    = ability.duplex;
    pPortability->link      = ability.link;
    pPortability->nway      = ability.nway;
...
rtk_api_ret_t dal_rtl8367c_port_macStatus_get(rtk_port_t port, rtk_port_mac_ability_t *pPortstatus)
{
    ...
    if (rtk_switch_chipType_get() == CHIP_RTL8370C)
    {
        if(status.speed >= 4)
        {
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_serdesReg_set(rtk_uint32 sdsID, rtk_uint32 page, rtk_uint32 reg, rtk_uint32 regData)
{
    rtk_api_ret_t retVal;
    rtk_uint32 cmdData;
    rtk_uint32 busy;
    ...
    switch (sdsID)
    {
        case 0:
            ...
            break;
        case 1:
            if ((rtk_switch_chipType_get() != CHIP_RTL8370B) && (rtk_switch_chipType_get() != CHIP_RTL8370C))
                return RT_ERR_INPUT;
            cmdData = 0x00C1;
            break;
        default:
    ...
    if ((retVal = rtl8367c_setAsicReg(RTL8367C_REG_SDS_INDACS_ADR, (page << 5) | reg)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(RTL8367C_REG_SDS_INDACS_CMD, cmdData)) != RT_ERR_OK)
        return retVal;
    ...
    do
    {
        ...
        if ((retVal = rtl8367c_getAsicRegBit(RTL8367C_REG_SDS_INDACS_CMD, RTL8367C_SDS_CMD_BUSY_OFFSET, &busy)) != RT_ERR_OK)
            return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_serdesReg_get(rtk_uint32 sdsID, rtk_uint32 page, rtk_uint32 reg, rtk_uint32 *pData)
{
    rtk_api_ret_t retVal;
    rtk_uint32 cmdData;
    rtk_uint32 busy;
    ...
    if ((retVal = rtl8367c_getAsicReg(RTL8367C_REG_SDS_INDACS_DATA, pData)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_serdesPolarity_set(rtk_uint32 sdsID, rtk_port_sdsPolarity_t inputPolarity, rtk_port_sdsPolarity_t outputPolarity)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    ...
    if ((retVal = dal_rtl8367c_port_serdesReg_get(sdsID, 0, 0, &regData)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = dal_rtl8367c_port_serdesReg_set(sdsID, 0, 0, regData)) != RT_ERR_OK)
        return retVal;
 ...
 *      This API can set serdes polarity of the specific sdsID.
 */
rtk_api_ret_t dal_rtl8367c_port_serdesPolarity_get(rtk_uint32 sdsID, rtk_port_sdsPolarity_t *pInputPolarity, rtk_port_sdsPolarity_t *pOutputPolarity)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_extPhyAutoPollingCfg_set(rtk_port_t port, rtk_port_extPhyAutoPollingCfg_t *pCfg)
{
    rtk_api_ret_t retVal;
    switch_chip_t chipID = 0;
    ...
    physicalPort = rtk_switch_port_L2P_get(port);
    ...
    /* State */
    if ((retVal = rtl8367c_setAsicRegBit(0x1e00, (extid + 12), (pCfg->state == ENABLED) ? 1 : 0))!=RT_ERR_OK)
        return retVal;

    /* PHY type */
    if ((retVal = rtl8367c_setAsicRegBits(0x1e00, 0x3 << (extid * 2 + 8), (pCfg->extPhyType & 0x3)))!=RT_ERR_OK)
        return retVal;    

    /* PHY ID */
    if ((retVal = rtl8367c_setAsicRegBits(0x1e01, 0x1F << extid * 5, pCfg->extPhyID))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_extPhyAutoPollingCfg_get(rtk_port_t port, rtk_port_extPhyAutoPollingCfg_t *pCfg)
{
    rtk_api_ret_t retVal;
    switch_chip_t chipID = 0;
    rtk_uint32 physicalPort;
    ...
    /* State */
    if ((retVal = rtl8367c_getAsicRegBit(0x1e00, (extid + 12), &regData))!=RT_ERR_OK)
        return retVal;
    
    pCfg->state = (regData == 1) ? ENABLED : DISABLED;

    /* PHY type */
    if ((retVal = rtl8367c_getAsicRegBits(0x1e00, 0x3 << (extid * 2 + 8), &regData))!=RT_ERR_OK)
        return retVal;    

    pCfg->extPhyType = (rtk_port_phy_type_t)regData;

    /* PHY ID */
    if ((retVal = rtl8367c_getAsicRegBits(0x1e01, 0x1F << extid * 5, &regData))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_extPhySmiState_set(rtk_enable_t state)
{
    rtk_api_ret_t retVal;
    switch_chip_t chipID = 0;
    ...
    if (state == ENABLED)
    {
        if ((retVal = rtl8367c_setAsicReg(0x1e48, 4))!=RT_ERR_OK)
            return retVal;
    }
    else
    {
        if ((retVal = rtl8367c_setAsicReg(0x1e48, 0))!=RT_ERR_OK)
            return retVal;
    }
 ...
 *      This API can set the state of SMI master.
 */
rtk_api_ret_t dal_rtl8367c_port_extPhySmiState_get(rtk_enable_t *pState)
{
    rtk_api_ret_t retVal;
    switch_chip_t chipID = 0;
    rtk_uint32 regData;
    ...
    if ((retVal = rtl8367c_getAsicReg(0x1e48, &regData))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_extPhyReg_set(rtk_uint32 phyID, rtk_port_phy_page_t page, rtk_port_phy_reg_t reg, rtk_port_phy_data_t data)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData, errInfo = 0, phyMsk = 0;
    rtk_uint32 busyCounter = 0xFFF;
    switch_chip_t chipID = 0;
    rtk_uint32 auto_polling8;
    rtk_uint32 auto_polling9;
    rtk_uint32 phyID8;
    rtk_uint32 phyID9;
    rtk_uint32 slot;
    ...
    if ((retVal = rtl8367c_setAsicRegBits(0x1e10, 0xFFF, page)) != RT_ERR_OK)
        return retVal;

    /* Auto parking back to page 0 */
    if ((retVal = rtl8367c_setAsicRegBits(0x1e0f, 0x1F, 0)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(0x1e13, data)) != RT_ERR_OK)
        return retVal;

    /* Looking for empty slot */
    if ((retVal = rtl8367c_getAsicReg(0x1e00, &regData))!=RT_ERR_OK)
        return retVal;

    auto_polling8 = (regData & 0x1000) ? 1 : 0;
    auto_polling9 = (regData & 0x2000) ? 1 : 0;

    if ((retVal = rtl8367c_getAsicReg(0x1e01, &regData))!=RT_ERR_OK)
        return retVal;
    ...
    if (slot == 0)
    {
        if (phyID8 != phyID)
        {
            regData &= ~0x001F;
            regData |= phyID;
            if ((retVal = rtl8367c_setAsicReg(0x1e01, regData))!=RT_ERR_OK)
                return retVal;
        }
    }
    else /* slot == 1 */
    ...
    /* Config to C22 mode */
    if ((retVal = rtl8367c_setAsicRegBits(0x1e00, 0x3 << (slot * 2 + 8), 0x3))!=RT_ERR_OK)
        return retVal;

    phyMsk = 0x0001 << slot;
    regData = (reg<<8) + (phyMsk << 3) + 5; 
    if ((retVal = rtl8367c_setAsicReg(0x1e0e, regData)) != RT_ERR_OK)
        return retVal;

    /* Polling status */
    while(busyCounter)
    {
        retVal = rtl8367c_getAsicRegBit(0x1e0e, 0, &regData);
        if(retVal != RT_ERR_OK)
            return retVal;
    ...
    if ((retVal = rtl8367c_getAsicRegBits(0x1e10, 0x7000, &errInfo)) != RT_ERR_OK)
        return retVal;
 ...
 *      This API can set register of external MDC/MDIO slave device.
 */
rtk_api_ret_t dal_rtl8367c_port_extPhyReg_get(rtk_uint32 phyID, rtk_port_phy_page_t page, rtk_port_phy_reg_t reg, rtk_port_phy_data_t *pData)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData, errInfo = 0;
    rtk_uint32 busyCounter = 0xFFF;
    switch_chip_t chipID = 0;
    rtk_uint32 auto_polling8;
    rtk_uint32 auto_polling9;
    ...
    if ((retVal = rtl8367c_setAsicReg(0x1e13, slot + 8)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicReg(0x1e0e, (reg << 8) + 0x1)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_getAsicReg(0x1e12, &regData)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367c_port_extC45PhyReg_set(rtk_uint32 phyID, rtk_uint32 mmd, rtk_port_phy_reg_t reg, rtk_port_phy_data_t data)
{
      
    rtk_api_ret_t retVal;
    rtk_uint32 regData, phyMsk = 0;
    rtk_uint32 errInfo = 0; 
    rtk_uint32 busyCounter = 0xFFF;
    switch_chip_t chipID = 0;
    rtk_uint32 auto_polling8;
    rtk_uint32 auto_polling9;
    ...
    if ((retVal = rtl8367c_setAsicRegBits(0x1e0f, 0x3E0, mmd)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(0x1e11, reg)) != RT_ERR_OK)
        return retVal;
    ...
    /* Config to C45 mode */
    if ((retVal = rtl8367c_setAsicRegBits(0x1e00, 0x3 << (slot * 2 + 8), 0x1))!=RT_ERR_OK)
        return retVal;
 ...
 *      This API can set register of external C45 MDC/MDIO slave device.
 */
rtk_api_ret_t dal_rtl8367c_port_extC45PhyReg_get(rtk_uint32 phyID, rtk_uint32 mmd, rtk_port_phy_reg_t reg, rtk_port_phy_data_t *pData)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData, errInfo = 0;
    rtk_uint32 busyCounter = 0xFFF;
    ...
    if ((retVal = rtl8367c_setAsicReg(0x1e48, 0x4)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicReg(0x1e0e, 0x3)) != RT_ERR_OK)
        return retVal;
