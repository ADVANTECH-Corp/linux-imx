...
static ssize_t proc_8367c_detect_write(struct file *file,
...
static int proc_8367c_detect_show(struct seq_file *m, void *v)
{
        seq_printf(m, "%s\n", mac_buf);
...
static int proc_8367c_detect_open(struct inode *inode, struct file *file)
{
        return single_open(file, proc_8367c_detect_show, NULL);
}

static const struct proc_ops proc_8367c_detect_fops = {
        .proc_open           = proc_8367c_detect_open,
        .proc_read           = seq_read,
        .proc_write          = proc_8367c_detect_write,
        .proc_lseek         = seq_lseek,
        .proc_release        = seq_release,
};

static int __init proc_8367c_detect_init(void)
{
        proc_create("8367c_detect", 0644, NULL, &proc_8367c_detect_fops);
        return 0;
}
fs_initcall(proc_8367c_detect_init);
