...
woal_priv_pcie_reg_rw(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	memset(data, 0, sizeof(data));
	header_len = strlen(CMD_MARVELL) + strlen(PRIV_CMD_PCIE_REG_RW);
	if (strlen(respbuf) == header_len) {
		PRINTM(MERROR, "Invalid number of parameters\n");
		ret = -EINVAL;
		goto done;
...
woal_priv_pcie_bar0_reg_rw(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	memset(data, 0, sizeof(data));
	header_len = strlen(CMD_MARVELL) + strlen(PRIV_CMD_PCIE_BAR0_REG_RW);
	if (strlen(respbuf) == header_len) {
		PRINTM(MERROR, "Invalid number of parameters\n");
		ret = -EINVAL;
		goto done;
	...
	if (strncmp(buf, CMD_MARVELL, strlen(CMD_MARVELL)) == 0) {
		...
		if (strnicmp
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_PCIE_REG_RW,
