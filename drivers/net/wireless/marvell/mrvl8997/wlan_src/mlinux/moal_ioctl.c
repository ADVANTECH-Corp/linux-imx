 ...
 */
mlan_status
woal_dpdlog_enble(moal_private *priv, t_u16 *enable,
		  t_u8 wait_option, t_u8 action)
{
	mlan_ioctl_req *req = NULL;
	mlan_ds_misc_cfg *cfg = NULL;
	...
	cfg = (mlan_ds_misc_cfg *)req->pbuf;
	cfg->sub_command = MLAN_OID_MISC_DPDLOG_ENABLE;
	req->req_id = MLAN_IOCTL_MISC_CFG;
	req->action = action;
	if (MLAN_ACT_SET == action)
		cfg->param.dpdlog_en = *enable;
	status = woal_request_ioctl(priv, req, wait_option);
	if (status != MLAN_STATUS_SUCCESS)
		goto done;
	if (MLAN_ACT_GET == action)
		*enable = cfg->param.dpdlog_en;
 ...
 */
mlan_status
woal_get_dpd_info(moal_private *priv, t_u8 wait_option,
		  mlan_ds_get_dpdlog * dpdlog)
{
	...
	/* Fill request buffer */
	info = (mlan_ds_get_info *)req->pbuf;
	info->sub_command = MLAN_OID_GET_DPD_LOG;
	req->req_id = MLAN_IOCTL_GET_INFO;
	req->action = MLAN_ACT_GET;
	...
	if (status == MLAN_STATUS_SUCCESS) {
		if (dpdlog)
			memcpy(dpdlog, &info->param.get_dpdlog,
			       sizeof(mlan_ds_get_dpdlog));
	}
done:
	if (status != MLAN_STATUS_PENDING)
		kfree(req);
