...
woal_send_tx_pkt_to_mon_if(moal_private *priv, pmlan_buffer pmbuf)
{
	...
	moal_recv_packet(handle, pmbuf2);
done:
	LEAVE();
}
#endif
...
#define DPD_LOG_MAX_SIZE     16000
#ifdef MFG_CMD_SUPPORT
#define MFG_DPD_LOG_MAX_SIZE 160000
#endif
#define DPD_LOG_SIZE         800

/**
 *  @brief  This function save the event to buf
 *
 *  @param phandle     A pointer to moal_handle
 *  @param buf         A point to buf
 *  @param pmevent     A pointer to mlan_event structure
 *
 *  @return            total length in buf
 */
static int
woal_dump_event_hex(moal_handle *phandle, t_u8 *buf, mlan_event *pmevent)
{
	char *ptr = NULL;
	moal_private *priv = NULL;
	t_u32 sec, usec;
	...
	if (!priv) {
		PRINTM(MMSG, "%s: can't get priv\n", __func__);
		LEAVE();
		return 0;
	}
	moal_get_system_time(phandle, &sec, &usec);
	ptr = (char *)buf;
	ptr += sprintf(ptr, "%u.%06u: %02x:%02x:%02x:%02x:%02x:%02x\n", sec,
		       usec, priv->current_addr[0], priv->current_addr[1],
		       priv->current_addr[2], priv->current_addr[3],
		       priv->current_addr[4], priv->current_addr[5]);
	//chop of event casue
	ptr += woal_save_hex_dump(ROW_SIZE_16, pmevent->event_buf + 4,
				  pmevent->event_len - 4, MFALSE, ptr);
	LEAVE();
 ...
 */
t_void
woal_store_dpd_log(moal_handle *phandle, mlan_event *pmevent)
{
	struct dentry *dentry;
	struct path path;
	struct file *pfile_dpdlog = NULL;
	loff_t pos = 0;
	t_u8 *buf = NULL;
	t_u32 len = 0;
	int ret;
...
save_dpdlog:
	ret = kern_path("/data/dpd_log.txt", 0, &path);
	if (!ret) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 11, 0)
		vfs_getattr(&path, &ks);
#else
		vfs_getattr(&path, &ks, STATX_SIZE, 0);
#endif
#ifdef MFG_CMD_SUPPORT
		if (mfg_mode)
			file_limit = MFG_DPD_LOG_MAX_SIZE;
#endif
		PRINTM(MDAT_D, "ks.size=%lld\n", ks.size);
		if ((ks.size + DPD_LOG_SIZE) > file_limit) {
			PRINTM(MMSG, "file over size, current=%lld limit=%d\n",
			       ks.size, file_limit);
			over_size = MTRUE;
		}
	}
	if (!over_size)
		...
		pfile_dpdlog =
			filp_open("/data/dpd_log.txt",
				  ...
				  0644);
	if (IS_ERR(pfile_dpdlog)) {
		PRINTM(MERROR, "Cannot create dpdlog file\n");
		LEAVE();
		return;
	}
	ret = moal_vmalloc(phandle, DPD_LOG_SIZE, &buf);
	if ((ret != MLAN_STATUS_SUCCESS) || !buf) {
		PRINTM(MERROR, "Error: vmalloc buf failed!\n");
		goto done;
	}
	len = woal_dump_event_hex(phandle, buf, pmevent);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0)
	vfs_write(pfile_dpdlog, buf, len, &pos);
#else
	kernel_write(pfile_dpdlog, buf, len, &pos);
#endif
done:
	filp_close(pfile_dpdlog, NULL);
	LEAVE();
	return;
}
...
MODULE_PARM_DESC(fw_region,
		 "1: Enable load channel regulatory domain, 0: Disable load channel regulatory domain");
