...
/**
 *  @brief Parse and print received diag loopback event
 *
 *  @param buffer   Pointer to the data buffer
 *  @param size     Length of the received event
 *  @return         N/A
 */
void
print_event_diag_loopback_report(t_u8 *buffer, t_u16 size)
{
	evt_diag_loopback *diag_event = (evt_diag_loopback *) buffer;

	switch (diag_event->test_item) {
	case DIAG_TEST_TX:
		printf("mode = TX mode\n");
		break;
	case DIAG_TEST_RX:
		...
		break;
	case DIAG_TEST_TXRX:
		printf("mode = TXRX mode\n");
		break;
	case DIAG_TEST_PINGPONG:
		printf("mode = PINGPONG mode\n");
		break;
	default:
		printf("Unknown mode\n");
		break;
	...
	switch (diag_event->event_type) {
	case EVENT_TYPE_WRONG_PKT:
		printf("------ Wrong Packet Report ------\n");
		if (diag_event->test_item == DIAG_TEST_PINGPONG) {
			printf("seq_num = %d\n",
			       diag_event->body.wrong_pkt_info.seq_num);
			printf("pkt_size = %d\n",
			       diag_event->body.wrong_pkt_info.pkt_size);
		} else
			printf("Wrong Packet event not for this mode\n");
		printf("---------------------------------\n");
		break;
	case EVENT_TYPE_RESULT:
		printf("------ Result Report ------ \n");
		if (diag_event->test_item == DIAG_TEST_TX) {
			printf("num_packet_tx = %d\n",
			       diag_event->body.result_info.num_packet_tx);
		} else if (diag_event->test_item == DIAG_TEST_RX) {
			printf("num_packet_rx = %d\n",
			       diag_event->body.result_info.num_packet_rx);
		} else if (diag_event->test_item == DIAG_TEST_TXRX) {
			printf("num_packet_tx = %d\n",
			       diag_event->body.result_info.num_packet_tx);
			printf("num_packet_rx = %d\n",
			       diag_event->body.result_info.num_packet_rx);
		} else if (diag_event->test_item == DIAG_TEST_PINGPONG) {
			printf("num_packet_succeed = %d\n",
			       diag_event->body.result_info.num_packet_rx -
			       diag_event->body.result_info.num_error);
			printf("num_packet_failed = %d\n",
			       diag_event->body.result_info.num_error);
		}
		printf("----------------------------\n");
		break;
	default:
		printf("Unknown event_type\n");
		break;
	}
...
print_event(event_header *event, t_u16 size)
{
	...
	case EVENT_NLIST_REPORT:
		...
		print_event_11k_nlist_report(event->event_data,
					     size - EVENT_ID_LEN);
		break;
	case EVENT_DIAG_LOOPBACK:
		printf("EVENT: diag loopback report\n");
		print_event_diag_loopback_report(event->event_data,
						 size - EVENT_ID_LEN);
		break;
