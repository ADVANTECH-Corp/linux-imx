...
static int process_diag_loopback_test(int argc, char *argv[]);
...
struct command_node command_list[] = {
	...
	{"diaglooptest", process_diag_loopback_test},
#if defined(SDIO_MULTI_PORT_TX_AGGR) || defined(SDIO_MULTI_PORT_RX_AGGR)
...
/**
 * @brief      Set diagnostic loopback test
 *
 * @param argc Number of arguments
 * @param argv Pointer to the arguments array
 *
 ...
 */
static int
process_diag_loopback_test(int argc, char *argv[])
{
	int ret = 0;
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd;
	struct ifreq ifr;
	evt_diag_loopback *diag_event = NULL;
	...
	if (strcmp(argv[3], "3") == 0) {
		diag_event = (evt_diag_loopback *) buffer;
		...
		case EVENT_TYPE_WRONG_PKT:
			...
			if (diag_event->test_item == DIAG_TEST_PINGPONG) {
				...
				printf("pkt_size = %d\n",
				       diag_event->body.wrong_pkt_info.
				       pkt_size);
			} else
