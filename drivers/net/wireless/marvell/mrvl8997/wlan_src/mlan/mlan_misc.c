...
/**
 *  @brief  IOCTL to enable/disable DPD log event
 *
 *  @param pmadapter	A pointer to mlan_adapter structure
 *  @param pioctl_req	A pointer to ioctl request buffer
 *
 ...
 */
mlan_status
wlan_dpdlog_enable(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
	mlan_private *pmpriv;
	...
	/* Send command to firmware */
	ret = wlan_prepare_cmd(pmpriv,
			       HostCmd_CMD_DPD_LOG_EN,
			       cmd_action,
			       0, (t_void *)pioctl_req, &misc->param.dpdlog_en);
 ...
 */
mlan_status
wlan_get_info_dpdlog(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
	pmlan_private pmpriv = MNULL;
	mlan_status ret = MLAN_STATUS_SUCCESS;
	...
	if (pioctl_req->buf_len < sizeof(mlan_ds_get_dpdlog)) {
		PRINTM(MERROR,
		       "MLAN IOCTL information buffer length is too short.\n");
		pioctl_req->data_read_written = 0;
		pioctl_req->buf_len_needed = sizeof(mlan_ds_get_dpdlog);
		pioctl_req->status_code = MLAN_ERROR_INVALID_PARAMETER;
