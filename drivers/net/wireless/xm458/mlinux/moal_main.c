...
static void woal_hang_work_queue(struct work_struct *work)
{
	...
	mlan_wifi6_ioctl(reset_handle->pmlan_adapter, NULL);
...
#define DEF_NOA_INTERVAL 100
	...
	if (MLAN_STATUS_SUCCESS == mlan_wifi6_register(&device, &pmlan))
...
void woal_free_moal_handle(moal_handle *handle)
{
	...
	if (handle->pmlan_adapter) {
		mlan_wifi6_unregister(handle->pmlan_adapter);
				  ...
				  t_u8 *offset_string, t_u8 *value_string)
{
	...
	pmlan_wifi6_ioctl_req ioctl_req = NULL;
	mlan_ds_reg_mem *reg = NULL;

	ENTER();

	/* Alloc ioctl_req */
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_reg_mem));
	...
	ioctl_req->req_id = mlan_wifi6_ioctl_REG_MEM;
...
static mlan_status woal_init_fw_dpc(moal_handle *handle)
{
	...
	if (handle->firmware) {
		...
		if (handle->params.fw_reload == FW_RELOAD_SDIO_INBAND_RESET)
			...
			fw.fw_reload = 0;
		wifi_status = 0;
		ret = mlan_wifi6_dnld_fw(handle->pmlan_adapter, &fw);
	...
	ret = mlan_wifi6_set_init_param(handle->pmlan_adapter, &param);
	ret = mlan_wifi6_init_fw(handle->pmlan_adapter);
	if (ret == MLAN_STATUS_FAILURE) {
		wifi_status = 2;
		goto done;
...
void woal_mlan_debug_info(moal_private *priv)
{
	...
	PRINTM(MERROR, "mlan_wifi6_rx_processing =%d\n", info->mlan_wifi6_rx_processing);
...
)
{
	...
	index = mlan_wifi6_select_wmm_queue(priv->phandle->pmlan_adapter,
				      priv->bss_index, tid);
...
static int woal_start_xmit(moal_private *priv, struct sk_buff *skb)
{
	...
	case MLAN_STATUS_PENDING:
		...
		if (atomic_read(&priv->phandle->tx_pending) >= MAX_TX_PENDING)
			woal_stop_queue(priv->netdev);
#endif /*#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,29)*/
		if (!mlan_wifi6_is_main_process_running(priv->phandle->pmlan_adapter))
			queue_work(priv->phandle->workqueue,
				   &priv->phandle->main_work);
		break;
	case MLAN_STATUS_SUCCESS:
...
/**
 *  @brief This function alloc mlan_wifi6_ioctl_req.
 *
 *  @param size	   buffer size to allocate
 *
 *  @return        mlan_wifi6_ioctl_req pointer or NULL
 */
pmlan_wifi6_ioctl_req woal_alloc_mlan_wifi6_ioctl_req(int size)
{
	mlan_wifi6_ioctl_req *req = NULL;
	gfp_t flag;
	...
	flag = (in_atomic() || irqs_disabled()) ? GFP_ATOMIC : GFP_KERNEL;
	req = kzalloc((sizeof(mlan_wifi6_ioctl_req) + size + sizeof(int) +
		       sizeof(wait_queue)),
		      flag);
	if (!req) {
	...
	req->pbuf = (t_u8 *)req + sizeof(mlan_wifi6_ioctl_req) + sizeof(wait_queue);
	req->buf_len = (t_u32)size;
	req->reserved_1 = (t_ptr)((t_u8 *)req + sizeof(mlan_wifi6_ioctl_req));
...
t_void woal_store_firmware_dump(moal_handle *phandle, mlan_event *pmevent)
{
	...
	if (type == DUMP_TYPE_ENDE) {
		...
		mlan_wifi6_pm_wakeup_card(phandle->pmlan_adapter, MFALSE);
	}
	LEAVE();
	return;
}
...
static int woal_dump_mlan_drv_info(moal_private *priv, t_u8 *buf)
{
	...
	ptr += sprintf(ptr, "mlan_wifi6_rx_processing =%d\n",
		       info->mlan_wifi6_rx_processing);
...
static int woal_netdev_poll_rx(struct napi_struct *napi, int budget)
{
	...
	mlan_wifi6_rx_process(handle->pmlan_adapter, &recv);
	if (recv < budget)
		napi_complete(napi);
	LEAVE();
	return recv;
...
t_void woal_rx_work_queue(struct work_struct *work)
{
	...
	mlan_wifi6_rx_process(handle->pmlan_adapter, NULL);
...
t_void woal_main_work_queue(struct work_struct *work)
{
	...
	mlan_wifi6_main_process(handle->pmlan_adapter);
...
err_init_fw:
	if ((handle->hardware_status == HardwareStatusFwReady) ||
		...
		handle->init_wait_q_woken = MFALSE;
		status = mlan_wifi6_shutdown_fw(handle->pmlan_adapter);
		if (status == MLAN_STATUS_PENDING)
...
mlan_status woal_remove_card(void *card)
{
	...
	if ((handle->hardware_status == HardwareStatusFwReady) ||
	    (handle->hardware_status == HardwareStatusReady)) {
		/* Shutdown firmware */
		PRINTM(MIOCTL, "mlan_wifi6_shutdown_fw.....\n");
		handle->init_wait_q_woken = MFALSE;
		...
		if (status == MLAN_STATUS_PENDING)
			wait_event_interruptible(handle->init_wait_q,
						 handle->init_wait_q_woken);
		PRINTM(MIOCTL, "mlan_wifi6_shutdown_fw done!\n");
	}
	/* Unregister mlan */
	if (handle->pmlan_adapter) {
		mlan_wifi6_unregister(handle->pmlan_adapter);
...
static int woal_reset_and_reload_fw(moal_handle *handle, t_u8 mode)
{
	...
	if (!IS_SD9098(handle->card_type) && !IS_SD9097(handle->card_type) &&
		...
		mlan_wifi6_pm_wakeup_card(handle->pmlan_adapter, MTRUE);
	...
	if (!IS_SD9098(handle->card_type) && !IS_SD9097(handle->card_type) &&
	    !IS_SD9177(handle->card_type))
		mlan_wifi6_pm_wakeup_card(handle->pmlan_adapter, MFALSE);
reload_fw:
	/* Download FW */
	ret = woal_request_fw(handle);
	if (ret) {
