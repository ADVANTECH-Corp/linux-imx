...
/** mlan_wifi6_ioctl_req data structure */
typedef struct _mlan_wifi6_ioctl_req {
	/** Pointer to previous mlan_wifi6_ioctl_req */
	struct _mlan_wifi6_ioctl_req *pprev;
	/** Pointer to next mlan_wifi6_ioctl_req */
	struct _mlan_wifi6_ioctl_req *pnext;
	...
	t_ptr reserved_1;
} mlan_wifi6_ioctl_req, *pmlan_wifi6_ioctl_req;
...
typedef struct _mlan_callbacks {
	...
	mlan_status (*moal_ioctl_complete)(t_void *pmoal,
					   pmlan_wifi6_ioctl_req pioctl_req,
					   mlan_status status);
...
/** Registration */
MLAN_API mlan_status mlan_wifi6_register(pmlan_device pmdevice,
				   t_void **ppmlan_adapter);

/** Un-registration */
MLAN_API mlan_status mlan_wifi6_unregister(t_void *padapter);

/** Firmware Downloading */
MLAN_API mlan_status mlan_wifi6_dnld_fw(t_void *padapter, pmlan_fw_image pmfw);

/** Custom data pass API */
MLAN_API mlan_status mlan_wifi6_set_init_param(t_void *padapter,
					 pmlan_init_param pparam);

/** Firmware Initialization */
MLAN_API mlan_status mlan_wifi6_init_fw(t_void *padapter);

/** Firmware Shutdown */
MLAN_API mlan_status mlan_wifi6_shutdown_fw(t_void *padapter);

/** Main Process */
MLAN_API mlan_status mlan_wifi6_main_process(t_void *padapter);

/** Rx process */
mlan_status mlan_wifi6_rx_process(t_void *padapter, t_u8 *rx_pkts);

/** Packet Transmission */
MLAN_API mlan_status mlan_wifi6_send_packet(t_void *padapter, pmlan_buffer pmbuf);
...
#if defined(SDIO) || defined(PCIE)
/** interrupt handler */
MLAN_API mlan_status mlan_wifi6_interrupt(t_u16 msg_id, t_void *padapter);
...
MLAN_API t_void mlan_wifi6_pm_wakeup_card(t_void *padapter, t_u8 keep_wakeup);

MLAN_API t_u8 mlan_wifi6_is_main_process_running(t_void *adapter);
#ifdef PCIE
MLAN_API t_void mlan_wifi6_set_int_mode(t_void *adapter, t_u32 int_mode,
				  t_u8 func_num);
#endif
/** mlan ioctl */
MLAN_API mlan_status mlan_wifi6_ioctl(t_void *padapter, pmlan_wifi6_ioctl_req pioctl_req);
/** mlan select wmm queue */
MLAN_API t_u8 mlan_wifi6_select_wmm_queue(t_void *padapter, t_u8 bss_num, t_u8 tid);
