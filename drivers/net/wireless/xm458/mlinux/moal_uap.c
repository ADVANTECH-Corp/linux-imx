...
static int woal_uap_addba_param(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_11n_cfg));
	if (ioctl_req == NULL) {
		LEAVE();
		return -ENOMEM;
	}
	...
	ioctl_req->req_id = mlan_wifi6_ioctl_11N_CFG;
...
static int woal_uap_deep_sleep(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_pm_cfg));
	if (ioctl_req == NULL) {
		LEAVE();
		return -ENOMEM;
	}
...
static int woal_uap_band_steer(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_band_steer_cfg));
	if (ioctl_req == NULL) {
		LEAVE();
		return -ENOMEM;
	}
...
static int woal_uap_beacon_stuck(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(
...
static int woal_uap_snmp_mib(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_snmp_mib));
	if (ioctl_req == NULL) {
		LEAVE();
		return -ENOMEM;
	}
	snmp = (mlan_ds_snmp_mib *)ioctl_req->pbuf;
	ioctl_req->req_id = mlan_wifi6_ioctl_SNMP_MIB;
	switch (param.oid) {
...
static int woal_uap_domain_info(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_11d_cfg));
	if (ioctl_req == NULL) {
		LEAVE();
		return -ENOMEM;
	}
	cfg11d = (mlan_ds_11d_cfg *)ioctl_req->pbuf;
	ioctl_req->req_id = mlan_wifi6_ioctl_11D_CFG;
...
static int woal_uap_dfs_testing(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_11h_cfg));
	if (ioctl_req == NULL) {
		LEAVE();
		return -ENOMEM;
	}
	cfg11h = (mlan_ds_11h_cfg *)ioctl_req->pbuf;
	ioctl_req->req_id = mlan_wifi6_ioctl_11H_CFG;
...
#define CMD_RESPBUF_LEN 2048
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_11ac_cfg));
	if (ioctl_req == NULL) {
		ret = -ENOMEM;
		goto done;
	}
	...
	ioctl_req->req_id = mlan_wifi6_ioctl_11AC_CFG;
...
static int woal_uap_tx_rate_cfg(struct net_device *dev, struct ifreq *req)
{
	...
	mlan_wifi6_ioctl_req *mreq = NULL;
	...
	mreq = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_rate));
	if (mreq == NULL) {
		ret = -ENOMEM;
		goto done;
	}
	...
	mreq->req_id = mlan_wifi6_ioctl_RATE;
...
static int woal_uap_antenna_cfg(struct net_device *dev, struct ifreq *req)
{
	...
	mreq = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_radio_cfg));
	if (mreq == NULL) {
		ret = -ENOMEM;
		goto done;
	}
	...
	mreq->req_id = mlan_wifi6_ioctl_RADIO_CFG;
...
static int woal_uap_dfs_repeater(struct net_device *dev, struct ifreq *req)
{
	...
	mreq = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_misc_cfg));
	if (mreq == NULL) {
		ret = -ENOMEM;
		goto done;
	}
	...
	mreq->req_id = mlan_wifi6_ioctl_MISC_CFG;
	mreq->action = MLAN_ACT_GET;
...
static int woal_uap_report_mic_ioctl(struct net_device *dev, struct ifreq *req)
{
	...
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_sec_cfg));
	if (ioctl_req == NULL) {
		ret = -ENOMEM;
		goto done;
	}
	...
	ioctl_req->req_id = mlan_wifi6_ioctl_SEC_CFG;
	ioctl_req->action = MLAN_ACT_SET;
