 ...
 *
 *  @param priv         A pointer to moal_private structure
 *  @param req          A pointer to mlan_wifi6_ioctl_req structure
 *  @param wait_option  Wait option
 *
 *  @return             N/A
 */
static inline mlan_status woal_wait_ioctl_complete(moal_private *priv,
						   mlan_wifi6_ioctl_req *req,
						   t_u8 wait_option)
{
	mlan_status status;
 ...
 *
 *  @param priv     A pointer to moal_private structure
 *  @param req      A pointer to mlan_wifi6_ioctl_req buffer
 *
 *  @return         MTRUE/MFALSE
 */
static inline t_bool woal_cac_period_block_cmd(moal_private *priv,
					       pmlan_wifi6_ioctl_req req)
{
	...
	case mlan_wifi6_ioctl_11D_CFG:
		...
		if (sub_command == MLAN_OID_11D_DOMAIN_INFO_EXT)
			ret = MTRUE;
		break;
	case mlan_wifi6_ioctl_MISC_CFG:
 ...
 */
mlan_status woal_request_ioctl(moal_private *priv, mlan_wifi6_ioctl_req *req,
			       t_u8 wait_option)
{
	...
	status = mlan_wifi6_ioctl(priv->phandle->pmlan_adapter, req);
...
{
	mlan_wifi6_ioctl_req *req = NULL;
	mlan_ds_bss *bss = NULL;
	mlan_status status;
	ENTER();

	/* Allocate an IOCTL request buffer */
	req = (mlan_wifi6_ioctl_req *)woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_bss));
	if (req == NULL) {
		status = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	moal_memcpy_ext(priv->phandle, &bss->param.mac_addr, priv->current_addr,
			...
			sizeof(mlan_802_11_mac_addr));
	req->req_id = mlan_wifi6_ioctl_BSS;
...
{
	int ret = 0;
	mlan_wifi6_ioctl_req *ioctl_req = NULL;
	mlan_ds_misc_cfg *misc = NULL;
	mlan_status status = MLAN_STATUS_SUCCESS;

	ENTER();

	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_misc_cfg));
	if (ioctl_req == NULL) {
		ret = -ENOMEM;
		goto done;
	}
	...
	ioctl_req->req_id = mlan_wifi6_ioctl_MISC_CFG;
			      ...
			      mlan_bss_info *bss_info)
{
	...
	/* Allocate an IOCTL request buffer */
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_get_info));
	if (req == NULL) {
		PRINTM(MERROR,
	...
	req->req_id = mlan_wifi6_ioctl_GET_INFO;
				...
				int *ie_len, t_u8 wait_option)
{
	...
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_misc_cfg));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_MISC_CFG;
	req->action = action;
			       ...
			       t_u8 wait_option, int *value)
{
	...
	/* Allocate an IOCTL request buffer */
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_snmp_mib));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_SNMP_MIB;
	req->action = action;
				  ...
				  mlan_power_cfg_t *power_cfg)
{
	...
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_power_cfg));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_POWER_CFG;
	req->action = action;
				    ...
				    t_u8 wait_option)
{
	...
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_pm_cfg));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_PM_CFG;
	req->action = action;
				   ...
				   mlan_rate_cfg_t *datarate)
{
	...
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_rate));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_RATE;
	req->action = action;
				     ...
				     mlan_fw_info *fw_info)
{
	...
	/* Allocate an IOCTL request buffer */
	req = (mlan_wifi6_ioctl_req *)woal_alloc_mlan_wifi6_ioctl_req(
				...
				mlan_debug_info *debug_info)
{
	...
	/* Allocate an IOCTL request buffer */
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(t_u32) +
...
#define MV_ETH_FRAME_LEN 1514
	...
	status = mlan_wifi6_send_packet(priv->phandle->pmlan_adapter, pmbuf);
	switch (status) {
	case MLAN_STATUS_PENDING:
			      ...
			      t_u8 *bss_role)
{
	...
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_bss));
	if (req == NULL) {
		ret = -ENOMEM;
		goto done;
	}
				 ...
				 t_u32 enable)
{
	...
	/* Allocate an IOCTL request buffer */
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_sec_cfg));
	if (req == NULL) {
		status = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_SEC_CFG;
	req->action = MLAN_ACT_SET;
				   ...
				   t_u32 *tx_bf_cap)
{
	...
	/* Allocate an IOCTL request buffer */
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_11n_cfg));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_11N_CFG;
 ...
 *
 *  @param priv     Pointer to moal_private structure
 *  @param req      Pointer to mlan_wifi6_ioctl_req structure
 *
 *  @return         N/A
 */
void woal_process_ioctl_resp(moal_private *priv, mlan_wifi6_ioctl_req *req)
{
	int cfg80211_wext;
	...
	switch (req->req_id) {
	case mlan_wifi6_ioctl_GET_INFO:
#ifdef STA_WEXT
#ifdef STA_SUPPORT
...
int woal_11h_channel_check_ioctl(moal_private *priv, t_u8 wait_option)
{
	...
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_11h_cfg));
	if (req == NULL) {
		ret = -ENOMEM;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_11H_CFG;
	req->action = MLAN_ACT_SET;
					  ...
					  mlan_ds_remain_chan *pchan)
{
	...
	ENTER();
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_radio_cfg));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_RADIO_CFG;
				...
				mlan_scan_resp *scan_resp)
{
	...
	/* Allocate an IOCTL request buffer */
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_scan));
	if (req == NULL) {
		status = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_SCAN;
	req->action = MLAN_ACT_GET;
			      ...
			      mlan_802_11_ssid *req_ssid)
{
	...
	/* Allocate an IOCTL request buffer */
	ioctl_req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_scan));
	if (ioctl_req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	if (req_ssid && req_ssid->ssid_len != 0) {
		/* Specific SSID scan */
		ioctl_req->req_id = mlan_wifi6_ioctl_SCAN;
		ioctl_req->action = MLAN_ACT_SET;
...
mlan_status woal_priv_qos_cfg(moal_private *priv, t_u32 action, char *qos_cfg)
{
	...
	req = woal_alloc_mlan_wifi6_ioctl_req(sizeof(mlan_ds_wmm_cfg));
	if (req == NULL) {
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	req->req_id = mlan_wifi6_ioctl_WMM_CFG;
	req->action = action;
	if (action == MLAN_ACT_SET) {
