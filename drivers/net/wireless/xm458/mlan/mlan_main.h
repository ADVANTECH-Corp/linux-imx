...
typedef struct {
	...
	pmlan_wifi6_ioctl_req pioctl_req_curr;
...
typedef struct _mlan_operations {
	...
	/** ioctl handler */
	mlan_status (*ioctl)(t_void *adapter, pmlan_wifi6_ioctl_req pioctl_req);
	/** cmd handler */
...
struct _cmd_ctrl_node {
	...
	/** Command parameter */
	t_void *pdata_buf;
	/** Pointer to mlan_wifi6_ioctl_req if command is from IOCTL */
	t_void *pioctl_buf;
...
typedef struct _mlan_adapter {
	...
	pmlan_wifi6_ioctl_req pwarm_reset_ioctl_req;
#endif
	/** SCAN IOCTL request buffer pointer */
	pmlan_wifi6_ioctl_req pscan_ioctl_req;
	/** DPD data pointer */
	t_u8 *pdpd_data;
	/** DPD data length  */
	t_u32 dpd_data_len;
...
#if defined(PCIE)
mlan_status wlan_misc_ssu(pmlan_adapter pmadapter, pmlan_wifi6_ioctl_req pioctl_req);
#endif
...
/** Disconnect */
mlan_status wlan_disconnect(mlan_private *pmpriv, mlan_wifi6_ioctl_req *pioctl_req,
