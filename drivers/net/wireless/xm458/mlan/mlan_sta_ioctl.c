...
static mlan_status wlan_misc_ioctl_country_code(pmlan_adapter pmadapter,
						mlan_wifi6_ioctl_req *pioctl_req)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
 ...
 * fail
 */
mlan_status wlan_find_bss(mlan_private *pmpriv, pmlan_wifi6_ioctl_req pioctl_req)
{
 ...
 * otherwise fail
 */
mlan_status wlan_ops_sta_ioctl(t_void *adapter, pmlan_wifi6_ioctl_req pioctl_req)
{
	...
	case mlan_wifi6_ioctl_11H_CFG:
		...
		break;
	case mlan_wifi6_ioctl_11AC_CFG:
		status = wlan_11ac_cfg_ioctl(pmadapter, pioctl_req);
		break;
	case mlan_wifi6_ioctl_11AX_CFG:
