...
{
	pmlan_wifi6_ioctl_req pioctl_buf;
	...
	while ((pioctl_buf = (pmlan_wifi6_ioctl_req)util_dequeue_list(
 ...
 *
 * mlan_status mlan_wifi6_register(
 ...
 * See Also
 *   mlan_wifi6_unregister
 */
mlan_status mlan_wifi6_register(pmlan_device pmdevice, t_void **ppmlan_adapter)
{
...
error:
	PRINTM(MINFO, "Leave mlan_wifi6_register with error\n");
 ...
 */
mlan_status mlan_wifi6_unregister(t_void *padapter)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 */
mlan_status mlan_wifi6_dnld_fw(t_void *padapter, pmlan_fw_image pmfw)
{
 ...
 *
 */
mlan_status mlan_wifi6_set_init_param(t_void *padapter, pmlan_init_param pparam)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	mlan_adapter *pmadapter = (mlan_adapter *)padapter;
 ...
 */
mlan_status mlan_wifi6_init_fw(t_void *padapter)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	mlan_adapter *pmadapter = (mlan_adapter *)padapter;
 ...
 */
mlan_status mlan_wifi6_shutdown_fw(t_void *padapter)
{
...
static t_void mlan_queue_rx_work(mlan_adapter *pmadapter)
{
	...
	/* Check if already processing */
	if (pmadapter->mlan_wifi6_rx_processing) {
 ...
 */
mlan_status mlan_wifi6_rx_process(t_void *padapter, t_u8 *rx_pkts)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	mlan_adapter *pmadapter = (mlan_adapter *)padapter;
	pmlan_callbacks pcb;
	...
	pcb = &pmadapter->callbacks;
	pcb->moal_spin_lock(pmadapter->pmoal_handle, pmadapter->prx_proc_lock);
	if (pmadapter->mlan_wifi6_rx_processing || pmadapter->rx_lock_flag) {
		pmadapter->more_rx_task_flag = MTRUE;
		pcb->moal_spin_unlock(pmadapter->pmoal_handle,
				      pmadapter->prx_proc_lock);
		goto exit_rx_proc;
	} else {
		pmadapter->mlan_wifi6_rx_processing = MTRUE;
		pcb->moal_spin_unlock(pmadapter->pmoal_handle,
				      pmadapter->prx_proc_lock);
	}
	if (rx_pkts)
...
rx_process_start:
	...
	pmadapter->mlan_wifi6_rx_processing = MFALSE;
	pcb->moal_spin_unlock(pmadapter->pmoal_handle,
			      pmadapter->prx_proc_lock);
exit_rx_proc:
	LEAVE();
 ...
 *  @return			MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status mlan_wifi6_main_process(t_void *padapter)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	mlan_adapter *pmadapter = (mlan_adapter *)padapter;
	pmlan_callbacks pcb;
...
exit_main_proc:
	if (pmadapter->hw_status == WlanHardwareStatusClosing)
		mlan_wifi6_shutdown_fw(pmadapter);
	LEAVE();
	return ret;
}
 ...
 */
mlan_status mlan_wifi6_send_packet(t_void *padapter, pmlan_buffer pmbuf)
{
	mlan_status ret = MLAN_STATUS_PENDING;
	mlan_adapter *pmadapter = (mlan_adapter *)padapter;
	mlan_private *pmpriv;
 ...
 * otherwise fail
 */
mlan_status mlan_wifi6_ioctl(t_void *adapter, pmlan_wifi6_ioctl_req pioctl_req)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
...
mlan_status mlan_recv(t_void *padapter, pmlan_buffer pmbuf, t_u32 port)
{
	...
	if (port == pmadapter->rx_cmd_ep) {
		...
		case MLAN_USB_TYPE_EVENT:
			...
			pmbuf->data_offset += MLAN_TYPE_LEN;
			pmbuf->data_len -= MLAN_TYPE_LEN;
			/* MOAL to call mlan_wifi6_main_process for processing */
			break;
		default:
 ...
 */
t_u8 mlan_wifi6_select_wmm_queue(t_void *padapter, t_u8 bss_num, t_u8 tid)
{
	mlan_adapter *pmadapter = (mlan_adapter *)padapter;
 ...
 */
mlan_status mlan_wifi6_interrupt(t_u16 msg_id, t_void *adapter)
{
 ...
 *  @return         N/A
 */
t_void mlan_wifi6_pm_wakeup_card(t_void *adapter, t_u8 keep_wakeup)
{
	mlan_adapter *pmadapter = (mlan_adapter *)adapter;
 ...
 *  @return         MTRUE/MFALSE
 */
t_u8 mlan_wifi6_is_main_process_running(t_void *adapter)
{
	mlan_adapter *pmadapter = (mlan_adapter *)adapter;
	pmlan_callbacks pcb = &pmadapter->callbacks;
	t_u8 ret = MFALSE;
 ...
 *  @return         N/A
 */
t_void mlan_wifi6_set_int_mode(t_void *adapter, t_u32 int_mode, t_u8 func_num)
{
	mlan_adapter *pmadapter = (mlan_adapter *)adapter;
	ENTER();
