				     ...
				     pmlan_wifi6_ioctl_req pioctl_req)
{
	...
	if (pioctl_req->action == MLAN_ACT_GET) {
		...
		debug_info->mlan_processing = pmadapter->mlan_processing;
		debug_info->mlan_wifi6_rx_processing = pmadapter->mlan_wifi6_rx_processing;
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
mlan_status wlan_set_drvdbg(pmlan_adapter pmadapter, pmlan_wifi6_ioctl_req pioctl_req)
{
	mlan_ds_misc_cfg *misc = (mlan_ds_misc_cfg *)pioctl_req->pbuf;
	mlan_status ret = MLAN_STATUS_SUCCESS;
