...
)
{
	...
	RTW_INFO("hal_CustomizedBehavior_8188EE(): RT Customized ID: 0x%02X\n", pHalData->CustomerID);
...
)
{
	...
	if (!AutoLoadFail) {
		/* VID, PID */
		pHalData->EEPROMVID = ReadLE2Byte(&hwinfo[EEPROM_VID_88EE]);
		pHalData->EEPROMDID = ReadLE2Byte(&hwinfo[EEPROM_DID_88EE]);
...
static u8 ReadAdapterInfo8188EE(PADAPTER Adapter)
{
...
)
{
	...
	if (count <= 0) {
		RTW_INFO("Failed to polling write LLT done at address %x!\n", address);
		status = _FAIL;
	}
...
static u32 InitMAC(IN	PADAPTER Adapter)
{
	...
	rtw_write8(Adapter, REG_CR + 1, 0x06);
	rtw_write8(Adapter, REG_CR + 2, 0x00);
...
static u32 rtl8188ee_hal_init(PADAPTER Adapter)
{
	...
	if (rtw_is_surprise_removed(Adapter)) {
		RTW_INFO("rtl8188ee_hal_init(): bSurpriseRemoved!\n");
		return _SUCCESS;
	}

	RTW_INFO("=======>rtl8188ee_hal_init()\n");

	/* rtl8188ee_reset_desc_ring(Adapter); */
	rtStatus = rtw_hal_power_on(Adapter);
	...
	/*  */
#if (HAL_MAC_ENABLE == 1)
	RTW_INFO("MAC Config Start!\n");
	rtStatus = PHY_MACConfig8188E(Adapter);
	if (rtStatus != _SUCCESS) {
		RTW_INFO("MAC Config failed\n");
		return rtStatus;
	}
	RTW_INFO("MAC Config Finished!\n");
	...
	/* phy_set_bb_reg(Adapter, rFPGA0_AnalogParameter2, BIT10, 1);   */ /* Aries mark, 2012/06/01, sync with windows driver */
#endif
	...
	if (pwrpriv->rfoff_reason > RF_CHANGE_BY_PS) {
		...
		RTW_INFO("InitializeAdapter8188EE(): Turn on  ----------\n");
...
#endif
	{
		...
		if (pwrpriv->rf_pwrstate == rf_on) {
			 ...
			 * phy_set_rf_path_switch_8188e(Adapter, pHalData->bDefaultAntenna);	 */ /* Wifi default use Main */
...
static u32 rtl8188ee_hal_deinit(PADAPTER Adapter)
{
	...
	if (Adapter->bHaltInProgress == _TRUE) {
		RTW_INFO("====> Abort rtl8188ee_hal_deinit()\n");
		return _FAIL;
	}
...
/*
 *	Description:
 *              ePHY Read operation on RTL8188EE
 *
 *	Created by Roger, 2013.05.08.
 *   */
u2Byte
hal_mdio_read_8188ee(
	IN	PADAPTER	Adapter,
	IN	u1Byte		Addr
)
{
	...
	rtw_write8(Adapter, REG_MDIO_CTL_8188E, Addr | BIT6);
	tmpU1b = rtw_read8(Adapter, REG_MDIO_CTL_8188E) & BIT6;
	count = 0;
	...
	if (tmpU1b == 0)
		ret = rtw_read16(Adapter, REG_MDIO_RDATA_8188E);
...
/*
 *	Description:
 *              ePHY Write operation on RTL8188EE
 *
 *	Created by Roger, 2013.05.08.
 *   */
VOID
hal_mdio_write_8188ee(
	IN	PADAPTER	Adapter,
	IN	u1Byte		Addr,
	IN	u2Byte		Data
)
{
	...
	rtw_write16(Adapter, REG_MDIO_WDATA_8188E, Data);
	rtw_write8(Adapter, REG_MDIO_CTL_8188E, Addr | BIT5);
	tmpU1b = rtw_read8(Adapter, REG_MDIO_CTL_8188E) & BIT5;
	count = 0;
	while (tmpU1b && count < 20) {
		rtw_udelay_os(10);
		tmpU1b = rtw_read8(Adapter, REG_MDIO_CTL_8188E) & BIT5;
...
/*
 *	Description:
 *              PCI configuration space read operation on RTL8188EE
 *
 *	Created by Roger, 2013.05.08.
 *   */
u1Byte
...
)
{
	...
	rtw_write16(Adapter, REG_DBI_ADDR_8188E, ReadAddr);
	rtw_write8(Adapter, REG_DBI_FLAG_8188E, 0x2);
	tmpU1b = rtw_read8(Adapter, REG_DBI_FLAG_8188E);
	count = 0;
	while (tmpU1b && count < 20) {
		rtw_udelay_os(10);
		tmpU1b = rtw_read8(Adapter, REG_DBI_FLAG_8188E);
	...
	if (tmpU1b == 0) {
		ReadAddr = REG_DBI_RDATA_8188E + Addr % 4;
...
/*
 *	Description:
 *              PCI configuration space write operation on RTL8188EE
 *
 *	Created by Roger, 2013.05.08.
 *   */
VOID
hal_dbi_write_8188ee(
	IN	PADAPTER	Adapter,
	IN	u2Byte		Addr,
	IN	u1Byte		Data
)
{
	...
	WriteAddr = REG_DBI_WDATA_8188E + Remainder;
	rtw_write8(Adapter, WriteAddr, Data);

	/* Write DBI 2Byte Address & Write Enable */
	WriteAddr = (Addr & 0xfffc) | (BIT0 << (Remainder + 12));
	rtw_write16(Adapter, REG_DBI_ADDR_8188E, WriteAddr);

	/* Write DBI Write Flag */
	rtw_write8(Adapter, REG_DBI_FLAG_8188E, 0x1);
...
u8 SetHwReg8188EE(PADAPTER Adapter, u8 variable, u8 *val)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	u8 ret = _SUCCESS;
	...
	case HW_VAR_SET_RPWM:
		rtw_write8(Adapter, REG_PCIE_HRPWM_8188E, *((u8 *)val));
		break;
	...
	case HW_VAR_DBI:
	{
		...
		pCmd = (u16 *)val;
		hal_dbi_write_8188ee(Adapter, pCmd[0], (u8)pCmd[1]);
	...
	case HW_VAR_MDIO:
	{
		...
		pCmd = (u16 *)val;
		hal_mdio_write_8188ee(Adapter, (u8)pCmd[0], pCmd[1]);
		break;
	}
...
void GetHwReg8188EE(PADAPTER Adapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_DBI:
		*val = hal_dbi_read_8188ee(Adapter, *((u16 *)(val)));
		break;
	case HW_VAR_MDIO:
		*((u16 *)(val)) = hal_mdio_read_8188ee(Adapter, *val);
		break;
	case HW_VAR_L1OFF_NIC_SUPPORT:
		{
		u8 l1off;

		l1off = hal_dbi_read_8188ee(Adapter, 0x160);
		if (l1off & (BIT2|BIT3))
			*val = _TRUE;
		else
			*val = _FALSE;
	...
	case HW_VAR_L1OFF_CAPABILITY:
		...
		l1off = hal_dbi_read_8188ee(Adapter, 0x15c);
		if (l1off & (BIT2|BIT3))
			*val = _TRUE;
		else
			*val = _FALSE;
...
void rtl8188ee_set_hal_ops(_adapter *padapter)
{
	...
	pHalFunc->set_hw_reg_handler = &SetHwReg8188EE;
	pHalFunc->GetHwRegHandler = &GetHwReg8188EE;
	pHalFunc->get_hal_def_var_handler = &GetHalDefVar8188EE;
