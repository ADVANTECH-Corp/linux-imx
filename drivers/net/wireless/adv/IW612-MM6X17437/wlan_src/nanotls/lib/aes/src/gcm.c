				     ...
				     // 128, 192 or 256-bit keys respectively)
{
	...
	if ((ret = aes_setkey(&ctx->aes_ctx, key, keysize)) != 0)
		return (ret);
...
int gcm_start(gcm_context *ctx, // pointer to user-provided GCM context
	      enum aes_op_mode mode, // GCM_ENCRYPT or GCM_DECRYPT
...
// TODO: cleanup
#ifndef AES_GCM_FIXED_12_BYTE_IV
	...
	if (iv_len == 12) { // GCM natively uses a 12-byte, 96-bit IV
		memcpy(ctx->y, iv, iv_len); // copy the IV to the top of the 'y'
					    // buff
		ctx->y[15] = 1; // start "counting" from 1 (not 0)
	}
#ifdef AES_GCM_FIXED_12_BYTE_IV
	uassert(iv_len == 12);
#else
	       ...
	       uint8_t *output) // pointer to destination data
{
	...
	uassert(ctx->mode == AES_MODE_ENCRYPT || ctx->mode == AES_MODE_DECRYPT);
	...
	while (length > 0) {
		...
		if (ctx->mode == AES_MODE_ENCRYPT) {
			for (i = 0; i < use_len; i++) {
				// XOR the cipher's ouptut vector (ectr) with
				// our input
				output[i] = (uint8_t)(ectr[i] ^ input[i]);
				// now we mix in our data into the
				// authentication hash. if we're ENcrypting we
				// XOR in the post-XOR (output) results, but if
				// we're DEcrypting we XOR in the input data
				ctx->buf[i] ^= output[i];
			}
		} else if (ctx->mode == AES_MODE_DECRYPT) {
			for (i = 0; i < use_len; i++) {
				// but if we're DEcrypting we XOR in the input
				// data first, i.e. before saving to ouput data,
				// otherwise if the input and output buffer are
				// the same (inplace decryption) we would not
				// get the correct auth tag
...
int gcm_crypt_and_tag(gcm_context *ctx, // gcm context with key already setup
		      enum aes_op_mode mode, // cipher direction: GCM_ENCRYPT or
					     // GCM_DECRYPT
		     ...
		     size_t tag_len) // byte length of the tag <= 16
{
	...
	*/
	gcm_crypt_and_tag(ctx, AES_MODE_DECRYPT, iv, iv_len, add, add_len,
