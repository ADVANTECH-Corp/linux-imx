 ...
 */
static uint32_t u256_add(uint32_t z[8], const uint32_t x[8],
			 const uint32_t y[8])
{
	uint32_t carry = 0;
 ...
 * Note: as a memory area, z must be either equal to x or y, or not overlap.
 */
static uint32_t u256_sub(uint32_t z[8], const uint32_t x[8],
			 const uint32_t y[8])
{
	uint32_t carry = 0;
 ...
 * Note: as a memory area, z must be either equal to x or y, or not overlap.
 */
static void m256_add(uint32_t z[8], const uint32_t x[8], const uint32_t y[8],
		     const m256_mod *mod)
{
	uint32_t r[8];
 ...
 * Note: as a memory area, z must be either equal to x or y, or not overlap.
 */
static void m256_sub(uint32_t z[8], const uint32_t x[8], const uint32_t y[8],
		     const m256_mod *mod)
{
	uint32_t r[8];
 ...
 */
static void m256_mul(uint32_t z[8], const uint32_t x[8], const uint32_t y[8],
		     const m256_mod *mod)
{
	/*
...
static void m256_inv(uint32_t z[8], const uint32_t x[8], const m256_mod *mod)
{
	 ...
	 *
	 * Take advantage of the fact that both p's and n's least significant
	 * limb is at least 2 to perform the subtraction on the flight (no
	 * carry).
	 *
 ...
 */
static int m256_from_bytes(uint32_t z[8], const uint8_t p[32],
			   const m256_mod *mod)
{
	u256_from_bytes(z, p);
...
{
	/*
	 * This is formula 6 from [CMO98], cited as complete in [RCB15] (table
	 * 1). Notations as in the paper, except u added and t ommited (it's
	 * x3).
	 */
		      ...
		      const uint32_t x2[8], const uint32_t y2[8])
{
	 ...
	 * intermediates with neutral names, and names from the paper in
	 * comments.
	 */
 ...
 */
static void point_add_or_double_leaky(uint32_t x3[8], uint32_t y3[8],
				      const uint32_t x1[8],
				      const uint32_t y1[8],
				      const uint32_t x2[8],
				      const uint32_t y2[8])
 ...
 */
static void point_to_bytes(uint8_t p[64], const uint32_t x[8],
			   const uint32_t y[8])
{
 ...
 */
static void scalar_mult(uint32_t rx[8], uint32_t ry[8], const uint32_t px[8],
			const uint32_t py[8], const uint32_t s[8])
{
	/*
	 ...
	 *      s_odd * P' = 2 * ( ... (2 * P' + sbit(b255) P') ... ) + sbit(b1)
	 * P'
	 *
 ...
 */
static int scalar_gen_with_pub(uint8_t sbytes[32], uint32_t s[8], uint32_t x[8],
			       uint32_t y[8])
{
 ...
 */
int p256_ecdh_shared_secret(uint8_t secret[32], const uint8_t priv[32],
			    const uint8_t peer[64])
{
	CT_POISON(priv, 32);
		      ...
		      const uint8_t *hash, size_t hlen)
{
	...
	if (u256_diff0(u1) == 0) {
		...
		scalar_mult(px, py, p256_gx, p256_gy, u1); /* (px, py) = R1 = u1
							      * G */

		/* (u1, u2) = R = R1 + R2 */
		point_add_or_double_leaky(u1, u2, px, py, e, s);
		/* No need to check if R == 0 here: if that's the case, it will
		 * be caught when comparating rx (which will be 0) to r (which
		 * isn't). */
	}
