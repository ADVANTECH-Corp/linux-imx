...
static char *mlancsi_help[] = {
	"Usage: ",
	"	mlancsi <interface> csi config/<csi_filter_file_name>",
	"	mlancsi <interface> event config/<csi_event_file_name>",
	"	where config/<xxx_file_name> is a configuration file.",
	"	when it is required to dump event's raw data to screen along with processing, use command:",
	"	mlancsi <interface> event config/<csi_filter_file_name> dump_to_screen",
	"	when it is required to dump event's raw data to file along with processing , use command:",
	"	mlancsi <interface> event config/<csi_filter_file_name> dump_to_file",
	"	mlancsi <interface> event",
	"	when no configuration file is given, the first CSI record is used as template.",
	"	when it is required to dump event's raw data to screen only, use command:",
	"	mlancsi <inerface> event dump_to_screen 1",
	"	when it is required to dump event's raw data to file along with dump to screen, use command:",
	"	mlancsi <inerface> event dump_to_file 1",
	"	file name will be csidump_<interface_name>.txt ",
	""};
...
t_u8 process_csi_event = 0;
t_u8 dump_to_file = 0;
t_u8 dump_to_screen = 0;
#define ARG_DUMP_TO_SCREEN "dump_to_screen"
#define ARG_DUMP_TO_FILE "dump_to_file"
/********************************************************
		Local Functions
********************************************************/
/**
...
#endif
					if (ret_val ==
					    MLAN_STATUS_FAILURE) { // done,
								   // terminate
								   // app
						gwls_data.terminate_app = 1;
					}
				}
				if (dump_to_screen || dump_to_file) {
					dump_raw_csi_event(event, bytes_read,
							   if_name,
							   dump_to_screen,
							   dump_to_file);
				}
...
static int process_event_cmd(int argc, char *argv[])
{
	...
	if (argc == 4) {
		...
		/* only dump the event to screen */
		if (!strncmp(argv[CSI_DUMP_ARG_INDEX], ARG_DUMP_TO_SCREEN,
			     strlen(ARG_DUMP_TO_SCREEN))) {
			if (IS_HEX_OR_DIGIT(argv[CSI_DUMP_OP_ARG_INDEX]) ==
			    MLAN_STATUS_FAILURE)
				ret = MLAN_STATUS_FAILURE;
			else
				dump_to_screen = (t_u8)A2HEXDECIMAL(
					argv[CSI_DUMP_OP_ARG_INDEX]);
		}
		/* only dump the event to the file */
		else if (!strncmp(argv[CSI_DUMP_ARG_INDEX], ARG_DUMP_TO_FILE,
				  strlen(ARG_DUMP_TO_FILE))) {
			if (IS_HEX_OR_DIGIT(argv[CSI_DUMP_OP_ARG_INDEX]) ==
			    MLAN_STATUS_FAILURE)
				ret = MLAN_STATUS_FAILURE;
			else
				dump_to_file = (t_u8)A2HEXDECIMAL(
					argv[CSI_DUMP_OP_ARG_INDEX]);
		}
		/* Process the event and dump to screen also */
		else if (!strncmp(argv[CSI_DUMP_OP_ARG_INDEX],
				  ARG_DUMP_TO_SCREEN,
				  strlen(ARG_DUMP_TO_SCREEN))) {
			ret = mlancsi_read_config(argv[CSI_CFG_FILE_ARG_INDEX]);
			process_csi_event = 1;
		...
		/* Process the event and dump to file also */
		else if (!strncmp(argv[CSI_DUMP_OP_ARG_INDEX], ARG_DUMP_TO_FILE,
				  strlen(ARG_DUMP_TO_FILE))) {
			ret = mlancsi_read_config(argv[CSI_CFG_FILE_ARG_INDEX]);
			process_csi_event = 1;
			dump_to_file = 1;
	...
	if ((ret == MLAN_STATUS_FAILURE) || (argc > 5)) {
