...
static int profile_read_block(FILE *fp, profile_entry_t *p_head)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (strstr(line, "}") != NULL) {
			...
			if (*ptr != '"') {
				p_head->psk_config = 1;
				memcpy((char *)p_head->psk, ptr, KEY_LEN);
			} else {
				eptr = strrchr(ptr + 1, '"');
				if (eptr == NULL) {
					fprintf(stderr,
...
int process_chanrpt(int argc, char *argv[])
{
	...
	/* buffer = MRVL_CMD<cmd> */
	strncpy((char *)buffer, CMD_NXP, strlen(CMD_NXP));
	strncpy((char *)buffer + strlen(CMD_NXP), HOSTCMD, strlen(HOSTCMD) + 1);
...
int process_get_assocrsp(int argc, char *argv[])
{
	...
	/* buffer = MRVL_CMD<cmd> */
	strncpy((char *)buffer, CMD_NXP, strlen(CMD_NXP) + 1);
