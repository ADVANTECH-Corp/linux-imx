...
perform_init:
	...
	if (card->dev->device == PCIE_DEVICE_ID_NXP_88W9098P_FN1)
		mlan_wifi6_set_int_mode(handle->pmlan_adapter, pcie_int_mode, 1);
	else
#endif
		/* Update pcie_int_mode in mlan adapter */
		mlan_wifi6_set_int_mode(handle->pmlan_adapter,
...
static irqreturn_t woal_pcie_interrupt(int irq, void *dev_id)
{
	...
	ret = mlan_wifi6_interrupt(0xffff, handle->pmlan_adapter);
...
static irqreturn_t woal_pcie_msix_interrupt(int irq, void *dev_id)
{
	...
	if (handle->second_mac)
		...
		PRINTM(MINTR, "*\n");
	ret = mlan_wifi6_interrupt(ctx->msg_id, handle->pmlan_adapter);
	queue_work(handle->workqueue, &handle->main_work);
...
#endif
		mlan_wifi6_set_int_mode(handle->pmlan_adapter, pcie_int_mode, 0);
...
#endif
	ENTER();
	mlan_wifi6_pm_wakeup_card(phandle->pmlan_adapter, MTRUE);
