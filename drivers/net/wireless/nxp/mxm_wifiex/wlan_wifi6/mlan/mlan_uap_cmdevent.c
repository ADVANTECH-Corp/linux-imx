					     ...
					     mlan_wifi6_ioctl_req *pioctl_buf)
{
	...
	case HOST_CMD_APCMD_SYS_CONFIGURE: {
		...
		if (!pioctl_buf || (pioctl_buf->req_id != mlan_wifi6_ioctl_MISC_CFG))
			break;
					      ...
					      t_void *pdata_buf)
{
	...
	if (pioctl_buf->req_id == mlan_wifi6_ioctl_BSS) {
		bss = (mlan_ds_bss *)pioctl_buf->pbuf;
	...
	} else if (pioctl_buf->req_id == mlan_wifi6_ioctl_MISC_CFG) {
		misc = (mlan_ds_misc_cfg *)pioctl_buf->pbuf;
					   ...
					   mlan_wifi6_ioctl_req *pioctl_buf)
{
	...
	if (pioctl_buf) {
		...
		if (pioctl_buf->req_id == mlan_wifi6_ioctl_MISC_CFG) {
			misc = (mlan_ds_misc_cfg *)pioctl_buf->pbuf;
			cust_ie = (mlan_ds_misc_custom_ie *)
					  sys_config->tlv_buffer;
					 ...
					 t_void *pcmd_buf, t_void *pioctl)
{
	...
	mlan_wifi6_ioctl_req *pioctl_buf = (mlan_wifi6_ioctl_req *)pioctl;
	mlan_adapter *pmadapter = pmpriv->adapter;
#ifdef SDIO
	int ctr;
#endif
