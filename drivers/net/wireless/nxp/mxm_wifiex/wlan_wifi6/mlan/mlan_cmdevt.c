...
static t_void wlan_dump_info(mlan_adapter *pmadapter, t_u8 reason)
{
	...
	PRINTM(MERROR, "mlan_wifi6_rx_processing =%d\n",
	       pmadapter->mlan_wifi6_rx_processing);
 ...
 *
 *  @param pmadapter    A pointer to mlan_adapter
 *  @param pioctl_req   A pointer to mlan_wifi6_ioctl_req buf
 *
 *  @return             A pointer to first entry match pioctl_req
 */
...
{
	cmd_ctrl_node *pcmd_node = MNULL;
	mlan_wifi6_ioctl_req *pioctl_buf = MNULL;
	ENTER();
	...
	while (pcmd_node != (cmd_ctrl_node *)&pmadapter->cmd_pending_q) {
		if (pcmd_node->pioctl_buf) {
			pioctl_buf = (mlan_wifi6_ioctl_req *)pcmd_node->pioctl_buf;
 ...
 *  @param pioctl_buf   A pointer to mlan_wifi6_ioctl_req structure
 *
 *  @return        MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_ret_host_cmd(pmlan_private pmpriv,
				     HostCmd_DS_COMMAND *resp,
				     mlan_wifi6_ioctl_req *pioctl_buf)
{
			     ...
			     t_void *pioctl_buf, t_void *pdata_buf)
{
	...
	pmlan_wifi6_ioctl_req pioctl_req = (mlan_wifi6_ioctl_req *)pioctl_buf;
...
{
	mlan_callbacks *pcb = (mlan_callbacks *)&pmadapter->callbacks;
	mlan_wifi6_ioctl_req *pioctl_req = MNULL;
	ENTER();

	if (pcmd_node == MNULL)
		goto done;
	if (pcmd_node->pioctl_buf) {
		pioctl_req = (mlan_wifi6_ioctl_req *)pcmd_node->pioctl_buf;
...
mlan_status wlan_process_cmdresp(mlan_adapter *pmadapter)
{
	...
	if (pmadapter->curr_cmd)
		if (pmadapter->curr_cmd->pioctl_buf != MNULL) {
			pioctl_buf = (mlan_wifi6_ioctl_req *)
	...
	if (pmadapter->curr_cmd) {
		cmd_ctrl_node *free_cmd = pmadapter->curr_cmd;
		pioctl_buf = (mlan_wifi6_ioctl_req *)pmadapter->curr_cmd->pioctl_buf;
		pmadapter->curr_cmd = MNULL;
 ...
 *
 *  @param pmadapter    A pointer to mlan_adapter
 *  @param pioctl_req   A pointer to pmlan_wifi6_ioctl_req
 *
					 ...
					 pmlan_wifi6_ioctl_req pioctl_req)
{
	...
	if (pmadapter->curr_cmd && pmadapter->curr_cmd->pioctl_buf) {
		pioctl_buf = (mlan_wifi6_ioctl_req *)pmadapter->curr_cmd->pioctl_buf;
		if (pioctl_buf->req_id == mlan_wifi6_ioctl_SCAN) {
				 ...
				 pmlan_wifi6_ioctl_req pioctl_req)
{
	...
	wlan_release_cmd_lock(pmadapter);
#ifdef STA_SUPPORT
	if (pioctl_req->req_id == mlan_wifi6_ioctl_SCAN) {
		if (pmadapter->scan_processing &&
		    pmadapter->ext_scan_type == EXT_SCAN_ENHANCE) {
			if (priv) {
				if (MLAN_STATUS_FAILURE ==
 ...
 *  @param pmpriv       A pointer to mlan_private structure
 *  @param resp         A pointer to HostCmd_DS_COMMAND
 *  @param pioctl_buf   A pointer to A pointer to mlan_wifi6_ioctl_req
 *
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
