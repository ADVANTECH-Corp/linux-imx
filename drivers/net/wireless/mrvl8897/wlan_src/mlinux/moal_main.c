...
/** Driver version */
char driver_version[] =
	"PCIE8897-%s-C" KERN_VERSION "15" MLAN_RELEASE_VERSION
	"-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
	"-dbg"
#endif
...
woal_main_work_queue(struct work_struct *work)
{
	...
	if (handle->pcie_fw_dbg == MTRUE) {
		handle->pcie_fw_dbg = MFALSE;
...
woal_interrupt(moal_handle *handle)
{
	...
	ret = mlan_interrupt(handle->pmlan_adapter);
