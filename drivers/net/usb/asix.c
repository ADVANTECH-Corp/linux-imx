...
static int ax8817x_set_mac_addr(struct net_device *net, void *p)
{
	...
	ret = ax8817x_write_cmd(dev, AX88772_CMD_WRITE_NODE_ID,
				 0, 0, ETH_ALEN, net->dev_addr_shadow);
	if (ret < 0)
		return ret;
	eth_hw_addr_set(net, net->dev_addr_shadow);
#else
	ret = ax8817x_write_cmd(dev, AX88772_CMD_WRITE_NODE_ID,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0)
	ret = ax8817x_write_cmd(dev, AX88772_CMD_WRITE_NODE_ID,
				 0, 0, ETH_ALEN, dev->net->dev_addr_shadow);
	if (ret < 0)
		deverr(dev, "set MAC address failed: %d", ret);
	eth_hw_addr_set(dev->net, dev->net->dev_addr_shadow);
#else
	ret = ax8817x_write_cmd(dev, AX88772_CMD_WRITE_NODE_ID,
...
#else
		eth_hw_addr_set(dev->net, g_mac_addr);
#endif
...
static int ax8817x_get_mac(struct usbnet *dev, u8* buf)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0)
		ret = access_eeprom_mac(dev, dev->net->dev_addr_shadow, 0x04, 1);
#else
	...
	ax8817x_write_cmd (dev, AX88772_CMD_WRITE_NODE_ID, 0, 0,
			   ETH_ALEN, dev->net->dev_addr_shadow);
	eth_hw_addr_set(dev->net, dev->net->dev_addr_shadow);
#else
	ax8817x_write_cmd (dev, AX88772_CMD_WRITE_NODE_ID, 0, 0,
...
static int ax88772b_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	*((u8 *)buf + 3) = 0x00;
	...
	ret = ax8817x_read_cmd(dev, AX_CMD_READ_EEPROM, 0x17, 0, 2,
			       (void *)tmp16);
	if (ret < 0) {
		deverr(dev, "read SROM address 17h failed: %d", ret);
		goto err_out;
	...
	if (*tmp16 == 0x100) {
		ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_GPIOS,
			(AXGPIOS_GPO1EN | AXGPIOS_GPO1), 0, 0, NULL);
		if (ret < 0)
			return ret;
	}
...
static void ax88772b_link_reset(struct work_struct *work)
{
	...
	case AX_SET_RX_CFG:
	{
		...
		if (!(bmcr & BMCR_SPEED100)) {
			mode &= ~AX88772_MEDIUM_100MB;	
			ax8817x_write_cmd(dev, AX_CMD_WRITE_GPIOS,
			(AXGPIOS_GPO1EN | 0x0), 0, 0, NULL);
		} else {
			mode &= ~AX88772_MEDIUM_100MB;	
			ax8817x_write_cmd(dev, AX_CMD_WRITE_GPIOS,
			(AXGPIOS_GPO1EN | AXGPIOS_GPO1), 0, 0, NULL);
