...
#define DRIVER_VERSION		"5.4.0 beta4"
...
static void axusbnet_bh_task(unsigned long param)
{
	struct usbnet		*dev = (struct usbnet *) param;
...
static void axusbnet_bh_tasklet(struct tasklet_struct *t)
{
	struct usbnet *dev = from_tasklet(dev, t, bh);

	axusbnet_bh(&dev->delay);
}
#endif
/*-------------------------------------------------------------------------*/
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 9, 0)
	tasklet_setup(&dev->bh, axusbnet_bh_tasklet);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0)
	tasklet_init(&dev->bh, axusbnet_bh_task, (unsigned long)dev);
