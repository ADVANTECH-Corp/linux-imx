...
char board_type[20];
char board_cpu[20];
char board_uboot_version[100]={'\0'};
char board_advboot_version[100]={'\0'};
...
static int proc_board_probe_dt(struct proc_board_data *board , struct platform_device *pdev)
{
	...
	printk("proc-board %s %s detected...\n", board_type, board_cpu);
#if 1
	board_info.uboot_version = of_get_property(np, "uboot_version", NULL);
	if (board_info.uboot_version) strcpy(board_uboot_version, board_info.uboot_version);
	if (board_info.advboot_version) strcpy(board_advboot_version, board_info.advboot_version);
#endif
