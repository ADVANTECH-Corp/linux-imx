...
#define QLOG_VERSION "1.5.13"  //when release, rename to V1.X
...
static int qlog_handle(const struct arguments *args) {
    ...
    while (qlog_exit_requested == 0) {
        ...
        if (g_is_unisoc_chip == 2)   //unisoc EC200D
        {
            if (rc >= 10)  // sizeof(apdump_recv_buf)
            {
                ...
                int i;
                for(i=0;i<sizeof(apdump_recv_buf);i++)
                {
        ...
        if (g_is_unisoc_exx00u_chip)  //check whether it is blue screen dump
        {
        ...
        if (fds[0] == dmfd) {
            g_unisoc_log_type = 0;
            wc = qlog_ops.logfile_save(logfile_fd, rbuf, rc);     //dm(cp log)
            savelog_size_dm += wc;
        }
            ...
            if (second_logfile_suffix) {
                if (args->ql_dev->general_type == MDM_QDSS)
                    ...
                    wc = qlog_ops.logfile_save(second_logfile, rbuf, rc); //ex00u ap log

                savelog_size_general += wc;
            }
            else {
                g_unisoc_log_type = 1;
                wc = qlog_ops.logfile_save(logfile_fd, rbuf, rc);     //rg500u log
                savelog_size_dm += wc;
           }
        }
    ...
    if (qlog_exit_requested && qlog_ops.clean_filter) {
        ...
        {
            qlog_ops.clean_filter(generalfd);
        }
        else
...
static struct arguments *parser_args(int argc, char **argv)
{
    ...
    while (-1 != (opt = getopt(argc, argv, "p:s:n:a:g:m:f:D::citqh")))
    {
        switch (opt)
        {
        case 'p':
...
{
    if ((idVendor == 0x2c7c && ((idProduct & 0xF000) == 0x6000))) // ASR
        return 1;
...
int drv_is_unisoc(int idProduct, int idVendor)
{
    if (idVendor == 0x2c7c && idProduct == 0x0900)
        ...
        && (idVendor == 0x3c93 && idProduct == 0xffff))
        return 1;
    else if (idVendor == 0x2c7c && idProduct == 0x0902)
        return 2;
...
int drv_is_unisoc_exx00u(int idProduct, int idVendor)
{
    ...
    if (idVendor == 0x2c7c && idProduct == 0x0904)   //EC800G
        return 2;
...
static int prepare(struct arguments *args)
{
    ...
    if (usb_dev->ttyDM[0] && !force_use_usbfs) {
        ...
        if (g_is_unisoc_exx00u_chip || g_is_unisoc_chip == 2)
