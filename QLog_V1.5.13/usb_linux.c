    ...
    while (NULL != (usb = readdir(usb_dir)))
    {
        ...
        if (ql_dev.bNumInterfaces > 1) {
            if (drv_is_asr(ql_dev.idProduct, ql_dev.idVendor)) { //ASR
                ...
                if (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x0904)
                {
                    ql_dev.dm_intf.bInterfaceNumber = 5;            //cplog_intf
                    ql_dev.general_intf.bInterfaceNumber = 3;       //aplog_intf
            ...
            else if (drv_is_mtk(ql_dev.idProduct,ql_dev.idVendor)) {  //mtk RM500K
...
_rescan_dm:
        ...
        else if (ql_dev.general_intf.bInterfaceNumber != 0xFF
            && (ql_dev.general_type == MDM_QDSS)) {
        ...
        qlog_dbg("Find [%d] idVendor=%04x, idProduct=%04x, bNumInterfaces=%d, ttyDM=%s, ttyGENERAL=%s, busnum=%03d, dev=%03d, usbdevice_pah=%s\n",
            modules_num, ql_dev.idVendor, ql_dev.idProduct, ql_dev.bNumInterfaces, ql_dev.ttyDM,
            ql_dev.ttyGENERAL, ql_dev.busnum, ql_dev.devnum, ql_dev.usbdevice_pah);
